...
st.write("Upload assets to automatically detect their properties and verify standard limits (Max 150 KB, Max 30s animation).")

# File uploader - Now accepts multiple files
uploaded_files = st.file_uploader("Upload Assets (JPG, PNG, GIF)", type=["jpg", "jpeg", "png", "gif"], accept_multiple_files=True)

if uploaded_files:
    st.write(f"### Processed {len(uploaded_files)} asset(s)")
    
    # Loop through every file uploaded
    for uploaded_file in uploaded_files:
        
        # Create a collapsible box for each file to keep the UI clean
        with st.expander(f"Report for: {uploaded_file.name}", expanded=True):
            results = {
                "Status": "✅ Pass",
                "File Type": "Unknown",
                "Size (KB)": "0",
            ...
            # Display Results inside the expander
