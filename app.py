...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Manrope', sans-serif; }
        ...
        th { background-color: #F1F5F9; color: #334155; padding: 14px 24px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em; text-align: left; border-bottom: 2px solid #E2E8F0; }
        td { padding: 18px 24px; font-size: 13px; color: #0F172A; text-align: left; border-bottom: 1px solid #E2E8F0; vertical-align: middle; word-wrap: break-word; font-weight: 500; }
    ...
    <script>
        ...
        function updateSummary() {
            ...
            if (totalCount > 0) {
                ...
                // Handle Empty States
                if (passCount === 0) {
                    document.getElementById('tbody-pass').innerHTML = "<tr class='empty-row'><td colspan='5'>No compliant assets in this batch.</td></tr>";
                }
                if (failCount === 0) {
                    document.getElementById('tbody-fail').innerHTML = "<tr class='empty-row'><td colspan='5'>✓ All assets passed. No action required.</td></tr>";
                }
            } else {
        ...
        // Logic QA: Timeout Protection for corrupted images
        function getImageDimensions(file) {
            const imgPromise = new Promise((resolve) => {
                const img = new Image();
                img.onload = () => resolve(`${img.width} × ${img.height}`);
                img.onerror = () => resolve("Unreadable / Corrupted");
                img.src = URL.createObjectURL(file);
            });
            const timeoutPromise = new Promise((resolve) => setTimeout(() => resolve("Timeout Reading File"), 2000));
            return Promise.race([imgPromise, timeoutPromise]);
        }

        async function handleFiles(files) {
            // UI QA: Show loading state
            ...
            // Allow UI to paint the loading text before locking up with processing
            await new Promise(resolve => setTimeout(resolve, 50)); 

            // Remove empty states before appending
            if (passCount === 0) document.getElementById('tbody-pass').innerHTML = "";
            if (failCount === 0) document.getElementById('tbody-fail').innerHTML = "";

            const allowedMimeTypes = ['image/jpeg', 'image/png', 'image/gif'];

            for (let file of files) {
                // Logic QA: Deduplication
                ...
                // Logic QA: Hard cap protection against massive files crashing the browser
                if (sizeKB > 5120) { // 5MB cap
                    status = "Fail";
                    errors.push("File exceeds 5MB hard limit (Browser protection)");
                    appendRow(file.name, ext, sizeStr, "Skipped", animationHtml, status, errors, sizeKB);
                    continue;
                }

                // Logic QA: Strict MIME Type validation
                ...
                if (dimensions.includes("Unreadable") || dimensions.includes("Timeout")) {
                    errors.push("Corrupted or unreadable image data");
                    status = "Fail";
                }

                if (ext === "GIF" && status !== "Fail") { // Only parse GIF if it isn't massive or corrupted
                ...
                appendRow(file.name, ext, sizeStr, dimensions, animationHtml, status, errors, sizeKB);
            }
        ...
        function appendRow(name, ext, sizeStr, dimensions, animationHtml, status, errors, sizeKB) {
            ...
            if (status === "Pass") {
                ...
                failCount++;
                statusBlock = `<div class='status-container'><div class='status-main status-text-fail'><span class='dot dot-fail'></span> Rejected</div><span class='text-error-detail'>${errors.join("<br>")}</span></div>`;
            }
