...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Manrope', sans-serif; }
        ...
        /* 6 Column Sizing */
        th:nth-child(1), td:nth-child(1) { width: 26%; font-weight: 700; } /* File Name */
        th:nth-child(2), td:nth-child(2) { width: 10%; } /* Format */
        th:nth-child(3), td:nth-child(3) { width: 10%; } /* Size */
        th:nth-child(4), td:nth-child(4) { width: 15%; } /* Dimensions */
        th:nth-child(5), td:nth-child(5) { width: 14%; } /* Animation */
        th:nth-child(6), td:nth-child(6) { width: 25%; } /* Status Report */
...
<body>
    ...
    <script>
        ...
        // The Standard Allowed Dimensions List
        ...
        function updateSummary() {
            ...
            if (totalCount > 0) {
                ...
                if (failCount === 0) document.getElementById('tbody-fail').innerHTML = "<tr class='empty-row'><td colspan='6'>✓ All assets passed. No action required.</td></tr>";
            } else {
        ...
        // Returns an object now so we have clean numbers for mathematical comparison
        ...
        async function handleFiles(files) {
            ...
            for (let file of files) {
                ...
                if (sizeKB > 5120) { 
                    ...
                    appendRow(file.name, ext, sizeStr, dimHtml, animationHtml, status, errors, sizeKB, false);
                    continue;
                }
                ...
                if (!imgInfo.valid) {
                    ...
                    let actualDimStr = `${imgInfo.width}x${imgInfo.height}`;
                    dimHtml = `<span class='text-primary'>${imgInfo.width} × ${imgInfo.height}</span>`;
                    
                    // 1. Check against standard whitelist
                    if (!STANDARD_DIMENSIONS.includes(actualDimStr)) {
                        errors.push(`Non-standard dimension (${actualDimStr})`);
                        status = "Fail";
                        dimFailed = true;
                    }

                    // 2. Extract dimensions from filename (e.g. campaign_728x90.jpg)
                    let nameRegex = /(?<!\d)(\d+)[xX](\d+)(?!\d)/;
                    let nameMatch = file.name.match(nameRegex);
                    if (nameMatch) {
                        let nameW = nameMatch[1];
                        let nameH = nameMatch[2];
                        let nameDimStr = `${nameW}x${nameH}`;
                        
                        // Compare extracted name dimensions to actual file dimensions
                        if (nameDimStr !== actualDimStr) {
                ...
                appendRow(file.name, ext, sizeStr, dimHtml, animationHtml, status, errors, sizeKB, dimFailed);
            }
        ...
        function appendRow(name, ext, sizeStr, dimHtml, animationHtml, status, errors, sizeKB, dimFailed) {
            let sizeColorClass = sizeKB > 150 ? 'text-error-detail' : 'text-primary';
            let formattedSize = `<span class='${sizeColorClass}'>${sizeStr}</span>`;
            
            // Turn the dimension red if it threw an error
            if (dimFailed) { dimHtml = dimHtml.replace('text-primary', 'text-error-detail'); }
